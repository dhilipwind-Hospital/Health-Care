...
// GET all payments
router.get('/payments/all', authenticate, isAdminOrFinance, (_req: Request, res: Response) => {
  // Map payments with invoice details
  ...
  res.json({ success: true, data: paymentsWithInvoice });
});
...
router.get('/stats/overview', authenticate, isAdminOrFinance, (_req: Request, res: Response) => {
  ...
  res.json({
    ...
    data: {
      ...
      pendingInvoices: invoices.filter(i => i.status === 'Sent' || i.status === 'Partially Paid').length,
