 ...
 *   post:
 *     summary: Check in a patient for their appointment (creates Visit + Queue token)
 *     tags: [Appointments]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *                 type: boolean
 *                 default: false
 *                 description: If true, patient goes directly to doctor queue
 *               priority:
 *                 type: string
 ...
 *                       type: string
 *       400:
 *         description: Invalid request or appointment not eligible for check-in
 *       404:
...
router.post(
  ...
  authenticate,
  tenantContext,
  errorHandler(AppointmentController.checkInPatient)
);
