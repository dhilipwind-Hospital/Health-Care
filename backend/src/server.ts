...
import patientAccessGrantRoutes from './routes/patient-access-grant.routes';
import locationRoutes from './routes/location.routes';
import phoneAuthRoutes from './routes/phone-auth.routes';
import salesInquiryRoutes from './routes/sales-inquiry.routes';
import deathCertificateRoutes from './routes/death-certificate.routes';
import birthRegisterRoutes from './routes/birth-register.routes';
import bloodbankRoutes from './routes/bloodbank.routes';
import dialysisRoutes from './routes/dialysis.routes';
import radiologyRoutes from './routes/radiology.routes';
import billingEnhancedRoutes from './routes/billing-enhanced.routes';
import consentRoutes from './routes/consent.routes';
import mlcRoutes from './routes/mlc.routes';
import drugRegisterRoutes from './routes/drug-register.routes';
import biomedicalWasteRoutes from './routes/biomedical-waste.routes';
import incidentReportRoutes from './routes/incident-report.routes';
...
export class Server {
  ...
  private initializeMiddlewares() {
    // Enable CORS - Must be first to handle preflight requests
    this.app.use(cors({
      origin: ['http://localhost:3000', 'http://localhost:5173', 'https://ayphen.com'],
      credentials: true,
  ...
  private initializeRoutes() {
    ...
    this.app.use('/api/phone-auth', phoneAuthRoutes);
    ...
    this.app.use('/api/birth-registers', birthRegisterRoutes);
    this.app.use('/api/blood-bank', bloodbankRoutes);
    this.app.use('/api/dialysis', dialysisRoutes);
    this.app.use('/api/radiology', radiologyRoutes);
    this.app.use('/api/billing-enhanced', billingEnhancedRoutes);
    this.app.use('/api/consent', consentRoutes);
    this.app.use('/api/mlc', mlcRoutes);
    this.app.use('/api/drug-register', drugRegisterRoutes);
    this.app.use('/api/biomedical-waste', biomedicalWasteRoutes);
    this.app.use('/api/incident-reports', incidentReportRoutes);
    this.app.use('/api/diet', dietRoutes);
    this.app.use('/api/assets', assetRoutes);
    this.app.use('/api/infection-control', infectionControlRoutes);
    this.app.use('/api/duty-roster', dutyRosterRoutes);
    this.app.use('/api/telemedicine', telemedicineRoutes);
    this.app.use('/api/abha', abhaRoutes);
    this.app.use('/api/pcpndt', pcpndtRoutes);
    ...
    this.app.get('/api/public/doctors', async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        // Priority: user's organizationId > tenant from context
        const orgId = (req as any).user?.organizationId || (req as any).tenant?.id;
        if (orgId) {
          qb.andWhere('u.organizationId = :orgId', { orgId });
        }
  ...
  public async start() {
    try {
      ...
      // Email service is already initialized in constructor
