 ...
 *  - there is any appointment history between them
 *  - there is an active cross-location access grant
 */
export async function canDoctorAccessPatient(patientId: string, doctorId: string): Promise<boolean> {
  ...
  // Cross-Location Access Grant check
  const crossOrgGrant = await PatientAccessGrantController.hasActiveAccess(doctorId, patientId);
  if (crossOrgGrant) return true;
