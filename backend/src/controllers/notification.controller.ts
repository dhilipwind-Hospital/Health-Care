...
export class NotificationController {
  ...
  static getMyNotifications = async (req: Request, res: Response) => {
    try {
      ...
      // Only filter by organization if tenantId is available
      if (tenantId) {
        queryBuilder.andWhere('notification.organization_id = :tenantId', { tenantId });
      }
      ...
      // Count unread - also filter by tenant if available
      const unreadQueryBuilder = repo.createQueryBuilder('notification')
        .where('notification.userId = :userId', { userId })
        .andWhere('notification.isRead = :isRead', { isRead: false });
      
      if (tenantId) {
        unreadQueryBuilder.andWhere('notification.organization_id = :tenantId', { tenantId });
      }
