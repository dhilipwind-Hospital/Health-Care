...
export class CallbackController {
  ...
  static createRequest = async (req: Request, res: Response) => {
    try {
      ...
      // Use tenant from user or header
      ...
      const repo = createTenantRepository(AppDataSource.getRepository(CallbackRequest), orgId);
      ...
      return res.status(500).json({ message: 'Error creating callback request' });
    }
  };
