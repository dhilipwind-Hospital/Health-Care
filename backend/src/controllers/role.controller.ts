...
export class RoleController {
  ...
  static async getAllRoles(req: Request, res: Response) {
    try {
      ...
      // Super admin can view roles without organization context
      ...
      let systemRoleCounts: any[] = [];
      if (tenantId) {
        systemRoleCounts = await userRepository
        ...
        // For super admin without org, get counts across all users
        systemRoleCounts = await userRepository
          .createQueryBuilder('user')
          .select('user.role', 'role')
          .addSelect('COUNT(*)', 'count')
      ...
      if (tenantId) {
        customRoles = await roleRepository
      ...
      const isSaaSAdmin = isSuperAdmin;
  ...
  static async getUserRoleAssignments(req: Request, res: Response) {
    try {
      ...
      // Super admin can view assignments without organization context
      if (!tenantId && !isSuperAdmin) {
        return res.status(400).json({ 
          success: false, 
          message: 'Organization context required' 
      ...
      if (tenantId) {
        ...
        // Super admin without org - get all users
        users = await userRepository.find({
          select: ['id', 'firstName', 'lastName', 'email', 'role', 'createdAt', 'isActive']
        });
      }
