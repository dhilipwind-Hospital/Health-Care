...
export class AppointmentController {
  ...
  static listDoctorAppointments = async (req: Request, res: Response) => {
    ...
    const qb = repo.createQueryBuilder('appointment')
      ...
      .addSelect(['patient.id', 'patient.firstName', 'patient.lastName', 'patient.email', 'patient.phone', 'patient.gender', 'patient.dateOfBirth'])
  ...
  static listAllAppointments = async (req: Request, res: Response) => {
    ...
    const qb = appointmentRepository
      ...
      .addSelect(['doctor.id', 'doctor.firstName', 'doctor.lastName', 'doctor.email'])
      .leftJoin('appointment.patient', 'patient')
      .addSelect(['patient.id', 'patient.firstName', 'patient.lastName', 'patient.email', 'patient.phone'])
      .orderBy('appointment.startTime', 'DESC')
      .skip(skip)
      .take(limitNum);
    ...
    // Filter by location if specified (join with users table to check location)
    if (locationId) {
      qb.andWhere('(doctor.location_id = :locationId OR patient.location_id = :locationId)', { locationId });
    }
  ...
  static cancelAppointment = async (req: Request, res: Response) => {
    ...
    try {
      ...
      try {
        const patient = appt.patient as User;
  ...
  // ==================== PATIENT CHECK-IN (Queue Integration) ====================
  /**
   * Check in a patient for their appointment - creates Visit + Queue token
   * This bridges Appointments to the Queue system without breaking existing flows
   */
  static checkInPatient = async (req: Request, res: Response) => {
    const { id } = req.params;
    const { skipTriage = false, priority = 'standard' } = req.body || {};
    const userId = (req as any).user?.id;
    const tenantId = (req as any).user?.organizationId || (req as any).tenant?.id;
    ...
    // Check if Queue module is enabled
    const queueEnabled = String(process.env.ENABLE_QUEUE || 'false').toLowerCase() === 'true';
    if (!queueEnabled) {
      return res.status(400).json({
        message: 'Queue module is not enabled. Set ENABLE_QUEUE=true in environment variables.',
        code: 'QUEUE_DISABLED'
      });
    }
    ...
    try {
      // 1. Fetch the appointment with relations
      ...
      // 2. Validate appointment is eligible for check-in
      ...
      if ((appointment as any).visitId) {
        return res.status(400).json({
          message: 'Patient has already checked in for this appointment',
          visitId: (appointment as any).visitId
        });
      }

      // 3. Validate appointment is for today (within reasonable window)
      const now = new Date();
      const appointmentDate = new Date(appointment.startTime);
      const hoursDiff = Math.abs(now.getTime() - appointmentDate.getTime()) / (1000 * 60 * 60);

      if (hoursDiff > 24) {
        return res.status(400).json({
          message: 'Can only check in within 24 hours of the appointment time',
          appointmentTime: appointment.startTime
        });
      }

      // 4. Import Visit-related entities (lazy to avoid circular deps)
      const { Visit } = await import('../models/Visit');
      const { QueueItem } = await import('../models/QueueItem');
      const { VisitCounter } = await import('../models/VisitCounter');
      ...
      const visitNumber = `V-${orgCode}-${today.slice(2)}-${String(visitSeq).padStart(4, '0')}`;
      const tokenNumber = `T-${orgCode}-${today.slice(2)}-${String(tokenSeq).padStart(4, '0')}`;

      // 6. Determine visit status and queue stage based on skipTriage
      const visitStatus = skipTriage ? 'with_doctor' : 'created';
      const queueStage = skipTriage ? 'doctor' : 'triage';
      const queuePriority = priority === 'emergency' ? 'emergency' : (priority === 'urgent' ? 'urgent' : 'standard');
      ...
      const queueItem = queueRepo.create({
        ...
        stage: queueStage as any,
        priority: queuePriority as any,
        tokenNumber,
        status: 'waiting' as any,
        assignedDoctorId: skipTriage && (appointment as any).doctor?.id ? (appointment as any).doctor.id : undefined
      });
      const savedQueue = await queueRepo.save(queueItem);

      // 9. Update appointment with visitId and status
      (appointment as any).visitId = (savedVisit as any).id;
      appointment.status = AppointmentStatus.IN_PROGRESS;
      await apptRepo.save(appointment);

      // 10. Add history entry
      await addHistory(
        appointment.id,
        'confirmed',
        `Patient checked in. Token: ${tokenNumber}. Stage: ${queueStage}`,
        String(userId)
      );

      // 11. INTEGRATION: Notify doctor that patient has checked in
      try {
        const doctorId = (appointment as any).doctor?.id;
        const patientName = `${(appointment as any).patient?.firstName || ''} ${(appointment as any).patient?.lastName || ''}`.trim() || 'Patient';
      ...
      return res.json({
        success: true,
        message: `Patient checked in successfully. Token: ${tokenNumber}`,
        data: {
          appointment: {
            id: appointment.id,
          ...
          visit: {
            id: (savedVisit as any).id,
            visitNumber,
            status: visitStatus
          },
          queueItem: {
            id: (savedQueue as any).id,
            stage: queueStage,
