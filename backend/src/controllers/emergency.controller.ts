...
export class EmergencyController {
  ...
  static createRequest = async (req: Request, res: Response) => {
    try {
      const { name, phone, location, message, priority } = req.body;
      ...
      const repo = createTenantRepository(AppDataSource.getRepository(EmergencyRequest), orgId);
      ...
      const newRequest = repo.create({
        ...
        location,
        message,
        priority: priority || EmergencyPriority.HIGH, // Default to HIGH for new requests from API
      ...
      return res.status(500).json({ message: 'Error creating emergency request' });
    }
  };
  ...
  static getDashboard = async (req: Request, res: Response) => {
    try {
      ...
      if (tenantId) {
        queryBuilder.where('emergency.organization_id = :tenantId', { tenantId });
      }
