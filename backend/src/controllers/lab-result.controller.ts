...
export class LabResultController {
  ...
  static enterLabResult = async (req: Request, res: Response) => {
    try {
      ...
      // Send notification for critical results to doctor
      if (flag === 'critical') {
        try {
      ...
      // Send notification to patient when all results are completed
      if (allCompleted) {
        try {
          const patient = orderItem.labOrder.patient;
          if (patient?.email) {
            await EmailService.sendTestResultNotificationEmail(
              patient.email,
              `${patient.firstName} ${patient.lastName}`,
              orderItem.labTest.name
            );
          }
        } catch (emailError) {
          console.error('Error sending patient result notification:', emailError);
        }
      }
  ...
  static verifyLabResult = async (req: Request, res: Response) => {
    try {
      ...
      // INTEGRATION: Notify patient that lab results are ready
      try {
        // Find the order item to get patient and test info
        const orderItem = await labOrderItemTenantRepo.findOne({
          where: { resultId: id },
          relations: ['labTest', 'labOrder', 'labOrder.patient']
        });

        if (orderItem && orderItem.labOrder && orderItem.labOrder.patient) {
          const patientId = orderItem.labOrder.patient.id;
          const testName = orderItem.labTest?.name || 'Lab Test';

          // Import notification service dynamically to avoid circular deps
          const { NotificationService } = await import('../services/notification.service');
          await NotificationService.notifyTestResultReady(patientId, testName);
        }
      } catch (notifyError) {
        console.error('Error sending test result notification:', notifyError);
