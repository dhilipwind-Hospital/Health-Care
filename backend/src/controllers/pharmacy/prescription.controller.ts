...
export class PrescriptionController {
  ...
  static createPrescription = async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        const savedItems = [];
        const stockWarnings: Array<{ medicineName: string; requested: number; available: number; status: 'low' | 'out_of_stock' }> = [];
        ...
        for (const item of items) {
          ...
          // INTEGRATION: Stock level check - warn doctor but don't block prescription
          ...
          if (medicine.currentStock === 0) {
          ...
          } else if (medicine.currentStock < requestedQty) {
            stockWarnings.push({
              medicineName: medicine.name,
              requested: requestedQty,
            ...
            // Warn if stock is at or below reorder level
            stockWarnings.push({
              medicineName: medicine.name,
              requested: requestedQty,
              available: medicine.currentStock,
        ...
        return res.status(201).json({
          message: stockWarnings.length > 0
            ? 'Prescription created with stock warnings'
      ...
      return res.status(500).json({ message: 'Failed to create prescription', error: (error as any).message });
    }
  };
  ...
  static getPatientPrescriptions = async (req: Request, res: Response) => {
    try {
      ...
      // Ensure we query correctly using relations
      const [prescriptions, total] = await prescriptionRepository.findAndCount({
        where: { patient: { id: patientId }, organization: { id: tenantId } },
      ...
      return res.status(500).json({ message: 'Failed to get patient prescriptions', error: (error as any).message });
    }
  };
  ...
  static getDoctorPrescriptions = async (req: Request, res: Response) => {
    try {
      ...
      const whereConditions: any = { doctor: { id: doctorId }, organization: { id: tenantId } };
      ...
      return res.status(500).json({ message: 'Failed to get doctor prescriptions', error: (error as any).message });
    }
  };
  ...
  static getPendingPrescriptions = async (req: Request, res: Response) => {
    try {
      ...
      const [prescriptions, total] = await prescriptionRepository.findAndCount({
        where: { status: PrescriptionStatus.PENDING, organization: { id: tenantId } },
        relations: ['doctor', 'patient', 'items', 'items.medicine'],
        skip,
        take,
      ...
      return res.status(500).json({ message: 'Failed to get pending prescriptions', error: (error as any).message });
    }
  };

  // Get all prescriptions (with status filter)
  static getAllPrescriptions = async (req: Request, res: Response) => {
    try {
      const user = (req as any).user;
      const tenantId = (req as any).tenant?.id || user?.organizationId;
      ...
      const whereConditions: any = { organization: { id: tenantId } };
      if (status) {
        whereConditions.status = status;
      }
      ...
      return res.status(500).json({ message: 'Failed to get prescriptions', error: (error as any).message });
    }
  };
  ...
  static dispensePrescription = async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        for (const itemUpdate of items) {
          ...
          const prescriptionItem = await prescriptionItemRepository.findOne({
            where: { id: itemUpdate.id, prescription: { id: id } }
          });
          ...
          if (itemUpdate.status === PrescriptionItemStatus.DISPENSED) {
            ...
            const medicine = await medicineRepository.findOne({
              where: { id: prescriptionItem.medicineId, organization: { id: tenantId } }
            });
        ...
        // INTEGRATION: Notify patient that prescription is ready
        if (allItemsDispensed) {
          try {
            const { NotificationService } = await import('../../services/notification.service');
            await NotificationService.notifyPrescriptionReady(prescription.patientId, prescription.id);
          } catch (notifyError) {
            console.error('Error sending prescription ready notification:', notifyError);
      ...
      return res.status(500).json({ message: 'Failed to dispense prescription', error: (error as any).message });
    }
  };
  ...
  static cancelPrescription = async (req: Request, res: Response) => {
    try {
      ...
      return res.status(500).json({ message: 'Failed to cancel prescription', error: (error as any).message });
    }
  };
}
