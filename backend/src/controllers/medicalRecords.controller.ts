...
import { checkDoctorPatientAccess } from '../utils/accessControl';
...
export const getMedicalRecords = async (req: Request, res: Response) => {
  try {
    ...
    // Determine effective organization ID (support cross-org access)
    let orgId = (req as any).tenant?.id || user?.organizationId;
    if ((req as any).crossOrgAccess) {
      orgId = (req as any).crossOrgAccess.patientOrganizationId;
    }
    ...
    // Filter by patient
    if (patientId) {
      // If doctor is requesting, check access permissions
      if (user.role === 'doctor') {
        // Skip standard check if we have explicit cross-org access
        if (!(req as any).crossOrgAccess) {
          const hasAccess = await checkDoctorPatientAccess(user.id, patientId as string, orgId);
          if (!hasAccess) {
            return res.status(403).json({ message: 'Access denied: You do not have permission to view this patient\'s records' });
          }
        }
      }
...
export const getMedicalRecord = async (req: Request, res: Response) => {
  try {
    ...
    if (user.role === 'doctor') {
      // Skip standard check if we have explicit cross-org access
      if (!(req as any).crossOrgAccess) {
        const hasAccess = await checkDoctorPatientAccess(user.id, record.patient.id, orgId);
        if (!hasAccess) {
          return res.status(403).json({ message: 'Access denied: You do not have permission to view this patient\'s records' });
        }
      }
