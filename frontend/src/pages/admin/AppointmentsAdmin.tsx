...
const AppointmentsAdmin: React.FC = () => {
  ...
  const onConfirmAppt = async (row: AppointmentRow) => {
    try {
      await api.post(`/appointments/admin/${row.id}/confirm`, {} as any);
  ...
  const onReschedule = async () => {
    ...
    try {
      await api.post(`/appointments/${rescheduleFor.id}/reschedule`, {
  ...
  useEffect(() => { loadRefs(); }, []);
  useEffect(() => { load(); }, [query, status, dateRange, departmentId, doctorId, patientId, page, selectedBranchId]);
