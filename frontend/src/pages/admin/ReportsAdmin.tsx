...
const ReportsAdmin: React.FC = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const locationId = selectedBranchId;
      const [statsData, deptData, activityData] = await Promise.all([
        analyticsService.getDashboardStats(locationId),
        analyticsService.getDepartmentPerformance(locationId),
        analyticsService.getRecentActivity(locationId)
      ]);
