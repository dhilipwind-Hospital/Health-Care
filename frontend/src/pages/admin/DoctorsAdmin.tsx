...
const DoctorsAdmin: React.FC = () => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      // Build params with location filtering
      const params: any = {};
      if (selectedBranchId) {
        params.locationId = selectedBranchId;
      }
      const res = await api.get('/users', { params: { ...params, role: 'doctor', limit: 100 }, suppressErrorToast: true } as any);
      const rows: Doctor[] = (res.data?.data || res.data || []).map((d: any) => ({
        ...d,
  ...
  useEffect(() => { load(); }, [query, department, role, selectedBranchId]);
