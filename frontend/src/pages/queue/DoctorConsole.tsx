...
const DoctorConsole: React.FC = () => {
  ...
  const fetchQueue = async () => {
    try {
      ...
      // Auto-recover active session if page reloaded
  ...
  return (
    <Space direction="vertical" style={{ width: '100%' }}>
      ...
      <Card title="Waiting List" extra={<Button onClick={onCallNext} type="primary">Call Next</Button>}>
        <Table rowKey="id" dataSource={queue.filter(q => q.status === 'waiting')} columns={columns} pagination={{ pageSize: 8 }} />
      </Card>

      {current && (
        <Card
          title={`Current: ${current.tokenNumber}`}
          extra={
            <Space>
              {current.appointment && (
                <Button
