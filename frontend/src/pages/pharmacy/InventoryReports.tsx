...
const InventoryReports: React.FC = () => {
  ...
  const loadExpiryReport = async () => {
    try {
      ...
      setExpiryReport(res.data.data || { expiring: [], expired: [], expiringCount: 0, expiredCount: 0 });
    } catch (error) {
      console.error('Error loading expiry report:', error);
      // Set empty data instead of showing error
      setExpiryReport({ expiring: [], expired: [], expiringCount: 0, expiredCount: 0 });
    } finally {
      setLoading(false);
    }
  };
  ...
  const loadReorderReport = async () => {
    try {
      ...
      setReorderReport(res.data || { data: [], total: 0, totalEstimatedCost: '0.00' });
    } catch (error) {
      console.error('Error loading reorder report:', error);
      // Set empty data instead of showing error
      setReorderReport({ data: [], total: 0, totalEstimatedCost: '0.00' });
    }
  };
