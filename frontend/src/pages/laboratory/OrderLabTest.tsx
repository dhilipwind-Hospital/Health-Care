...
const OrderLabTest: React.FC = () => {
  ...
  const fetchTests = async () => {
    try {
      ...
      // Handle both response formats: { tests: [...] } or direct array
      const finalTestsData = Array.isArray(testsData) ? testsData : testsData.tests || [];
      setTests(finalTestsData);
      console.log('Tests data:', finalTestsData);

      if (finalTestsData.length === 0) {
  ...
  const fetchPatients = async () => {
    try {
      ...
      if (patientsData.length === 0) {
        message.warning('No patients found. Please treat patients in the Doctor Console first, or ensure you have appointments.');
      }
    } catch (error) {
      ...
      <Card style={{ marginBottom: 24 }}>
        <Form form={form} layout="vertical" onFinish={handleSubmit}>
          ...
          <Form.Item>
            ...
            <Button style={{ marginLeft: 8 }} onClick={() => navigate('/laboratory/results')}>
              Cancel
            </Button>
          </Form.Item>
        </Form>
