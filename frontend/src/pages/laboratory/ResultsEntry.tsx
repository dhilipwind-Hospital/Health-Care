...
const ResultsEntry: React.FC = () => {
  ...
  const fetchPendingResults = async () => {
    try {
      ...
      // Filter orders that have items with sample_collected status and a sampleId
      const ordersWithSamples = response.data.filter((order: any) => 
        order.items.some((item: any) => item.status === 'sample_collected' && (item.sample || item.sampleId))
      );
          ...
          {selectedOrder && (
            <>
              ...
              >
                ...
                >
                  {selectedOrder.items
                    .filter(item => item.status === 'sample_collected' && (item.sample || item.sampleId))
                    .map(item => (
                      <Option key={item.id} value={item.id}>
                        {item.labTest.name} ({item.labTest.code}) - Sample: {item.sample?.sampleId || item.sampleId}
                      </Option>
                    ))}
                </Select>
              </Form.Item>
              ...
              >
                <Descriptions column={1} size="small">
                  ...
                  <Descriptions.Item label="Sample ID">
                    <Tag color="blue">{selectedItem.sample?.sampleId || selectedItem.sampleId || 'N/A'}</Tag>
                  </Descriptions.Item>
                </Descriptions>
              </Card>
