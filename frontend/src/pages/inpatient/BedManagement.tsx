...
const BedManagement: React.FC = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'patient',
      render: (record: Bed) => {
        if (record.status !== 'available' && record.currentAdmission && record.currentAdmission.patient) {
          return (
            <div>
              <div>{`${record.currentAdmission.patient.firstName || ''} ${record.currentAdmission.patient.lastName || ''}`.trim() || 'Unknown'}</div>
